---
nameJa: 朝鮮舞踊部
nameEn: ''
category: 
  - Music & Arts
  - Dance & Performance
description: 朝鮮舞踊部です♪
thumbnail: 'https://firebasestorage.googleapis.com/v0/b/strait-infomation.firebasestorage.app/o/images%2Fclubs%2F2026_koreandance.jpeg?alt=media&token=791b448f-0fcb-4257-8141-047b5cb2d923'
overview:
  philosophy: 朝鮮舞踊を通して、朝鮮半島の文化や伝統を知り、楽しく踊ること。
  activities: |-
    主に4月の舞踊系合同新歓、11月の外語祭のダンスフェスティバルに出演しています！
    部員の希望や意欲があれば、外部のイベントに出演することもあります。
    チマ・チョゴリを着て、曲に合わせて扇や小太鼓、ショールなどを使って踊ります。
operations:
  executiveMembers:
    - 部員が少ないので、全員で一体となって運営しています。
  organization: 部長、副部長(主将)、庶務
membership:
  memberCount: 3
  yearDistribution:
    - '1年生: 2人'
    - '4年生: 1人'
  isIntraUniversity: false
schedule:
  frequency: 週2日
  location: サークル棟5階武道場板間
  annualPlan:
    - 4月の舞踊系合同新歓
    - 11月の外語祭
recruitment:
  applicationDeadline: ''
  appeal: |-
    少人数で、部員全員の都合に合わせて和気藹々と活動できるところが魅力です！
    また、朝鮮大学校を除けばおそらく日本の大学で唯一の朝鮮舞踊団体です！
  challenges: 部員が少ないので、運営が少し大変なのと、今後の引き継ぎが不安ではあります…新入生がたくさん入部してくれると助かります！
  welcomeEvents: |-
    主に新歓に力を入れているのは4,5月ですが、年間を通して部員を募集しています！
    4月の外新歓、舞踊系合同新歓に参加します。
    いつでも活動時間中に見学・体験を行っているので、お気軽にインスタやXのDMまたはGmailでご連絡ください！
  annualFee: 部費は徴収していません。イベント出演時に衣装のレンタル代を都度徴収しています。
  hasSelection: false
  targetAudience: 全学年、未経験者歓迎
  contact:
    facebook: ''
    website: ''
    line: ''
    instagram: 'https://www.instagram.com/tufsmuyong/'
    xUrl: 'https://x.com/tufsmuyong1'
  targetGrades:
    - 1年生
    - 2年生
    - 3年生
    - 4年生
    - 院生
  applicationFlow: |-
    主に新歓に力を入れているのは4,5月ですが、年間を通して部員を募集しています！
    4月の外新歓、舞踊系合同新歓に参加します。
    いつでも活動時間中に見学・体験を行っているので、お気軽にインスタやXのDMまたはGmailでご連絡ください！
lastUpdated: '2026-04-06'
---

