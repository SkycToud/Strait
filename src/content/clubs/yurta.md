---
nameJa: 中央ユーラシア同好会ユルタ
nameEn: Yurta
thumbnail: 'https://firebasestorage.googleapis.com/v0/b/strait-infomation.firebasestorage.app/o/images%2Fclubs%2F2026_yurta.jpeg?alt=media&token=a1bdb5d2-7bd0-4aaf-bcdc-1ff133e71c27'
category:
  - Planning
description: 中央ユーラシアの民族文化が好きな人、集まれ！
overview:
  philosophy: "活動は不定期です！兼部OK・参加自由\U0001F646‍♀️\n大語科から小語科まで、色んな語科のメンバーが在籍しています！2年生以上の入部も大歓迎です\U0001F64C"
  activities: 外語祭でユルタを建て、中央ユーラシアから輸入してきた物品を販売します。
  guidelines: 準備中
operations:
  executiveMembers:
    - 執行代は主に5回生4年生が担っています！
  organization: 部長、副部長、会計、広報など。
membership:
  memberCount: 32
  yearDistribution:
    - '2年生: 5人'
    - '3年生: 7人'
    - '4年生: 17人'
    - '院生: 2人'
  isIntraUniversity: true
schedule:
  frequency: 不定期(年に数回程度)
  location: 未定
  annualPlan:
    - 11月　外語祭にてユルタ設営・物販販売
recruitment:
  appeal: ゆるく活動したい人におすすめです！様々な語科からのメンバーを募集しております！
  challenges: "ユルタを建てるのがちょっとした重労働です\U0001F4A6"
  welcomeEvents: 入会フォームにご回答→メールアドレスにお送りするグループLINEのQRコードからご参加→入会完了
  applicationFlow: 入会フォームにご回答→メールアドレスにお送りするグループLINEのQRコードからご参加→入会完了
  applicationDeadline: 通年募集
  annualFee: 無
  hasSelection: false
  targetAudience: 全学年
  targetGrades:
    - 1年生
    - 2年生
    - 3年生
    - 4年生
    - 院生
  contact:
    instagram: 'https://www.instagram.com/projectyurta?igsh=MWpnaWd1M3k2dXk3eQ%3D%3D&utm_source=qr'
    xUrl: 'https://x.com/tufs_yurt?s=21&t=7En9ii6Rbfwiw6rNKzTE-A'
    form: 'https://forms.gle/GPATGpNRRkAwXAMv5'
lastUpdated: '2026-04-09'
---

