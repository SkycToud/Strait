---
nameJa: タップダンスサークルTUPS
nameEn: TUPS
category: Dance & Performance
description: 「楽しく”タップダンス”してみませんか？」週に１度サークル棟で活動しているアットホームなインカレサークルです
thumbnail: 'https://firebasestorage.googleapis.com/v0/b/strait-infomation.firebasestorage.app/o/images%2Fclubs%2F2026_tups.jpg?alt=media&token=a5b5b26c-56e3-4211-89f5-ed4025339aef'
xUrl: 'https://x.com/tufs_tups'
overview:
  philosophy: |-
    楽しく！自由に！タップダンス！
    ・週一の活動で程よい練習量
    ・初心者でも上達しやすい
    ・レッスンで技術も身につく
    ・お金があまりかからない
  guidelines: 準備中
  activities: |-
    ・週金曜日に集まって基礎ステップなどを練習
    ・外語祭シーズンは何曲か通しで練習（練習量も少し増えます）
    ・月に一度先生をお呼びしてレッスンをしています（参加は任意です）
operations:
  executiveMembers:
    - 学部２年生
  organization: 代表、副代表
membership:
  memberCount: 14
  yearDistribution:
    - '1年生: 3人'
    - '2年生: 2人'
    - '3年生: 3人'
    - '4年生: 6人'
  isIntraUniversity: false
schedule:
  frequency: 週一
  location: サークル棟多目的スペース
  annualPlan:
    - 11月　外語祭に出演
recruitment:
  applicationDeadline: ''
  appeal: |-
    ・初心者が多くアットホームな雰囲気
    ・お金があまりかからない
    ・留学に行くメンバーも多く、練習もスケジュールを調整するのでのんびり活動できます
    ・タップダンスはどんな曲にも合うダンスなので自分の好きな曲で踊れるところも魅力
  challenges: |-
    ・最初は足が痛くなりがち
    ・たまに全然できないステップがあり苦戦する
    ・サークル棟、夏暑すぎる
    ・外語祭時期はやっぱり少し大変
  welcomeEvents: 各種SNS(インスタ、X)のDMで随時受け付けています。体験希望や入会希望の旨を送ってください。
  annualFee: '部費はなし ・タップシューズ代（ものにもよるが大体　10000円ほど） レッスン代（1回2000 円前後） ・外語祭参加費（約1,500円） ・衣装を購入する場合は衣装代も'
  hasSelection: false
  targetAudience: 全学年
  contact:
    facebook: ''
    website: ''
    line: ''
    instagram: 'https://www.instagram.com/tufs_tups?igsh=MXZoY3dyNmplNDA2aQ=='
    xUrl: 'https://x.com/tufs_tups'
  targetGrades:
    - 1年生
    - 2年生
    - 3年生
    - 4年生
lastUpdated: '2026-04-05'
---

